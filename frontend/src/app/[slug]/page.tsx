...
export default function TenantLandingPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    const api = createPublicApi(slug);
    Promise.allSettled([api.getBranding(), api.getAccommodationTypes()]).then(
      ([brandingResult, typesResult]) => {
        if (brandingResult.status === "fulfilled") {
          setBranding(brandingResult.value.data);
        } else {
          const status = (brandingResult.reason as { response?: { status?: number } })?.response?.status;
  ...
  const handleSearch = async (data: PublicAvailabilityRequest) => {
    ...
    try {
      const res = await createPublicApi(slug).checkAvailability(data);
      setResults(res.data);
    } catch {
      setSearchError(
  ...
  if (notFound) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-klyp-pale gap-4 px-4 text-center">
        <p className="text-2xl font-bold text-klyp-navy">Empresa no encontrada</p>
        <p className="text-klyp-gray">
          El alojamiento <strong>{slug}</strong> no existe o no está disponible.
        </p>
        <Button asChild variant="outline">
          <Link href="/">Ver todos los alojamientos</Link>
        </Button>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-3">
            {logoUrl ? (
              ...
              <img
                src={logoUrl}
                alt={displayName}
                className="h-16 sm:h-20 w-auto max-w-[240px] sm:max-w-[320px] object-contain"
              />
            ) : (
              <>
        ...
        {results !== null && !isSearching && results.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-16 text-center">
            ...
            <p className="text-klyp-gray text-sm max-w-md">
              No hay alojamientos disponibles para las fechas y número de
              personas indicados. Prueba con otras fechas o modifica el número
              de personas.
            </p>
          </div>
        )}
        ...
        {results !== null && !isSearching && results.length > 0 && (
          <>
            <p className="mb-4 text-sm text-klyp-gray">
              {results.length} tipo{results.length !== 1 ? "s" : ""} de
              alojamiento disponible{results.length !== 1 ? "s" : ""}
            </p>
        ...
        {results === null && !isSearching && !searchError && (
          <div className="flex flex-col items-center gap-2 py-16 text-center">
            <p className="text-klyp-gray text-sm">
              Selecciona tus fechas y pulsa &ldquo;Buscar disponibilidad&rdquo;
              para ver los alojamientos.
            </p>
          </div>
        )}
      </main>
